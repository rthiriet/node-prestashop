...
prestashop.prototype.categoryList = function(){
    var self = this;
    console.log('get categories');
    var client = new rest();
    client.getRest('categories','xml')
        .on('successcategories',function(categoriesRaw){
            self.categories = categoriesRaw;
            inspect(self.categories);
            var x;
            var categoryArray= new Array();
            for(x=0; x < self.categories.categories.category.length; x++){
                categoryArray[x]=self.categories.categories.category[x]['@'].id;
            }
            self.receivedAndProcessed('categoryListReceived',categoryArray);
        })
